...
-- ============================================================
-- Creates two schemas (target + source) with controlled
...
CREATE TABLE target.pricing_rules (
...
CREATE TABLE source.pricing_rules (LIKE target.pricing_rules INCLUDING ALL);

-- target data (the "target" — current state)
INSERT INTO target.pricing_rules (id, rule_name, product_type, discount_rate, min_quantity, max_quantity, is_active, metadata) VALUES
    ...
    -- Row 6 will be DELETED in source (exists only in target)
...
-- source data (the "source" — admin's modifications)
INSERT INTO source.pricing_rules (id, rule_name, product_type, discount_rate, min_quantity, max_quantity, is_active, metadata) VALUES
    -- Row 1: UNCHANGED
    (1, 'Early Bird Discount',     'electronics', 0.1000, 1,   NULL, TRUE,  '{"campaign": "spring_2026"}'),
    ...
    -- Row 5: UNCHANGED
    (5, 'VIP Member Discount',     'all',         0.0500, 1,   NULL, TRUE,  '{"membership": "vip"}'),
    -- Row 6: DELETED (not present in source)
    -- Row 7: UNCHANGED
    (7, 'Student Discount',        'books',       0.1200, 1,   NULL, TRUE,  '{"requires_id": true}'),
...
CREATE TABLE target.discount_tiers (
    id              SERIAL PRIMARY KEY,
...
CREATE TABLE source.discount_tiers (LIKE target.discount_tiers INCLUDING ALL);

-- target
INSERT INTO target.discount_tiers (id, tier_name, min_spend, max_spend, discount_pct, applies_to) VALUES
...
-- source — admin raised all thresholds and discount_pct for Gold/Platinum
INSERT INTO source.discount_tiers (id, tier_name, min_spend, max_spend, discount_pct, applies_to) VALUES
...
CREATE TABLE target.shipping_rules (
    id              SERIAL PRIMARY KEY,
...
CREATE TABLE source.shipping_rules (LIKE target.shipping_rules INCLUDING ALL);

-- target (existing rules)
INSERT INTO target.shipping_rules (id, zone_name, country_code, base_cost, free_above, delivery_days, carrier) VALUES
    (1, 'France Metro',     'FR', 4.99,  50.00,  2, 'Colissimo'),
    (2, 'Germany Standard', 'DE', 6.99,  75.00,  3, 'DHL'),
    (3, 'UK Express',       'GB', 9.99,  100.00, 2, 'Royal Mail');

-- source — admin adds 3 new zones, keeps existing ones intact
INSERT INTO source.shipping_rules (id, zone_name, country_code, base_cost, free_above, delivery_days, carrier) VALUES
...
CREATE TABLE source.tax_rules (LIKE target.tax_rules INCLUDING ALL);

-- target (6 rules, admin will remove 3 deprecated ones)
INSERT INTO target.tax_rules (region_code, product_category, tax_rate, tax_name, effective_from, effective_to, is_active) VALUES
...
-- source — admin keeps only the 3 active rules
INSERT INTO source.tax_rules (region_code, product_category, tax_rate, tax_name, effective_from, effective_to, is_active) VALUES
    ('FR',    'electronics',  0.2000, 'TVA Standard',           '2024-01-01', NULL,         TRUE),
    ('FR',    'food',         0.0550, 'TVA Réduit',             '2024-01-01', NULL,         TRUE),
    ('FR',    'books',        0.0550, 'TVA Réduit Livres',      '2024-01-01', NULL,         TRUE);
...
CREATE TABLE target.no_change_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(100) NOT NULL,
...
CREATE TABLE source.no_change_rules (LIKE target.no_change_rules INCLUDING ALL);

INSERT INTO target.no_change_rules (id, rule_name, value, is_active) VALUES
    (1, 'Identical Rule A', 10.00, TRUE),
    (2, 'Identical Rule B', 20.00, FALSE),
    (3, 'Identical Rule C', 30.00, TRUE);

INSERT INTO source.no_change_rules (id, rule_name, value, is_active) VALUES
    (1, 'Identical Rule A', 10.00, TRUE),
    (2, 'Identical Rule B', 20.00, FALSE),
    (3, 'Identical Rule C', 30.00, TRUE);
...
CREATE TABLE target.nullable_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(100) NOT NULL,
...
CREATE TABLE source.nullable_rules (LIKE target.nullable_rules INCLUDING ALL);

INSERT INTO target.nullable_rules (id, rule_name, optional_a, optional_b, notes) VALUES
...
INSERT INTO source.nullable_rules (id, rule_name, optional_a, optional_b, notes) VALUES
...
CREATE TABLE target.jsonb_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(100) NOT NULL,
    config      JSONB NOT NULL
);

CREATE TABLE source.jsonb_rules (LIKE target.jsonb_rules INCLUDING ALL);

INSERT INTO target.jsonb_rules (id, rule_name, config) VALUES
    (1, 'JSONB Nested Test', '{"threshold": 100, "nested": {"enabled": true, "level": 1}, "tags": ["a", "b"]}'),
    (2, 'JSONB Unchanged',   '{"simple": "value"}');

INSERT INTO source.jsonb_rules (id, rule_name, config) VALUES
...
-- ============================================================
-- TABLE 8: empty_target_rules
-- Scenario: table vide côté target → tout est INSERT
-- ============================================================

CREATE TABLE target.empty_target_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(100) NOT NULL,
    value       NUMERIC(10,2) NOT NULL
);

CREATE TABLE source.empty_target_rules (LIKE target.empty_target_rules INCLUDING ALL);

-- target: vide (0 lignes)

-- source: 3 lignes — toutes seront des INSERTs
INSERT INTO source.empty_target_rules (id, rule_name, value) VALUES
    (1, 'New Rule Alpha',   10.00),
    (2, 'New Rule Beta',    20.00),
    (3, 'New Rule Gamma',   30.00);

-- ============================================================
-- TABLE 9: empty_source_rules
-- Scenario: table vide côté source → tout est DELETE
-- ============================================================

CREATE TABLE target.empty_source_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(100) NOT NULL,
    value       NUMERIC(10,2) NOT NULL
);

CREATE TABLE source.empty_source_rules (LIKE target.empty_source_rules INCLUDING ALL);

-- target: 3 lignes — toutes seront des DELETEs
INSERT INTO target.empty_source_rules (id, rule_name, value) VALUES
    (1, 'Deprecated Rule X', 11.00),
    (2, 'Deprecated Rule Y', 22.00),
    (3, 'Deprecated Rule Z', 33.00);

-- source: vide (0 lignes)
...
CREATE TABLE target.quoted_rules (
    id          SERIAL PRIMARY KEY,
    rule_name   VARCHAR(200) NOT NULL,
    description TEXT
);

CREATE TABLE source.quoted_rules (LIKE target.quoted_rules INCLUDING ALL);

INSERT INTO target.quoted_rules (id, rule_name, description) VALUES
    (1, 'Rule with ''single'' quotes', 'It''s a test with apostrophe'),
    (2, 'Rule unchanged',              'No special chars');

INSERT INTO source.quoted_rules (id, rule_name, description) VALUES
...
-- Grant access
-- ============================================================
GRANT USAGE ON SCHEMA target TO diffly;
GRANT USAGE ON SCHEMA source TO diffly;
GRANT SELECT ON ALL TABLES IN SCHEMA target TO diffly;
GRANT SELECT ON ALL TABLES IN SCHEMA source TO diffly;
